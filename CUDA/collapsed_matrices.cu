...
void vector2D_add_equals(Vector2D* a, const Vector2D* b) {
    a->x = a->x + b->x;
    a->y = a->y + b->y;
}
...
void vector2D_subtract_equals(Vector2D* a, const Vector2D* b) {
    a->x = a->x - b->x;
    a->y = a->y - b->y;
}
...
void Vector2D_to_float2 (Vector2D* vector2d, float2* float2d, int length) {
    for (int i = 0; i < length; i++)
    {
        float2d[i].x = vector2d[i].x;
        float2d[i].y = vector2d[i].y;
    }
}
