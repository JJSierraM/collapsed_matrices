...
struct CollapsedMatrix
{
    ...
    CollapsedMatrix(ushort dimension_v, ushort n_v) : 
        dimension(dimension_v),
        n(n_v),
        length(nCr(n, dimension)),
        indices((uint*) malloc(length*dimension*sizeof(uint))),
    ...
    void apply_function(T f(const U*), const U* items){
        ...
        for (uint i=0; i<length; i++){
            ...
            for (uint index=0; index<dimension; index++){
                selected_items[index] = items[indices[(uint)i*dimension+index]];
    ...
    void apply_function_2(T f(const U*, const uint*), const U* items){
        uint* selected_items = (uint*) malloc(dimension*sizeof(uint));
    ...
    //A ELIMINAR #########################################################
    static void print_arr(uint* arr, uint dim){
        for (int i=0; i<dim; i++) printf("%d",arr[i]);
        printf("\n");
    }
    // ###################################################################
    void transfer_array(uint* arr1, uint* arr2, uint dim_arr2){
...
static Vector2 grav_force_2(const Body* bodies, const uint* indices){
    const float GRAV_CONST = 5;
...
static Vector2 grav_force(const Body body_A, const Body body_B) {
    const float GRAV_CONST = 5;
    const Vector2 vec12 = body_A.position-body_B.position;
    const float force_mag = -GRAV_CONST * (body_A.mass*body_B.mass)/vec12.sqr_length();
    const float force_angle = vec12.angle();
    return Vector2::from_magnitude_and_angle(force_mag, force_angle);
}

void apply_grav_force(CollapsedMatrix<Vector2,Body>* collapsed_matrix, const Body* items){
...
void apply_grav_force_2(CollapsedMatrix<Vector2,Body>* collapsed_matrix, Body* items){
    int length = collapsed_matrix->length;
    //printf("Length applied: %d\n", length);
    int dimension = collapsed_matrix->dimension;
    //printf("Dimension applied: %d\n", dimension);
    ...
    #pragma omp parallel shared (GRAV_CONST)
    {
        Vector2 vec12;
        float force_mag=0.0;
        float force_angle=0.0;
...
int main () {
    ...
    // apply_grav_force_2(&collapsed_matrix, bodies);
    Vector2 auxiliary;
    for (int i=1; i<n; i++) {
        for (int j=0; j<i; j++) {
            auxiliary = grav_force(bodies[i], bodies[j]);
            bodies[i].accel += auxiliary;
            bodies[j].accel += auxiliary;
        }
    }
    ...
    end_fun = clock();
    start_sum = clock();
    collapsed_matrix.calculate_sum();
    end_sum = clock();
